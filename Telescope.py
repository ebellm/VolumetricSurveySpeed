...
class Telescope:
        ...
        if type(self.transmission) == float:
            ...
            try:
                ...
                raise NotImplementedError(
                    'Transmission not specified for given filter')
        else:
    ...
    def AB_to_Rstar(self, source_mag, filterkey='gprime', airmass=1.,
        ...
        # scale to aperture electrons at 1.36 FWHM: 71.5%  (LSST S/N doc eqn
        # 18)
        if aperture_cut:
            if aperture == 'aper':
    ...
    def sky_electrons_per_pixel(self, mag_per_sq_arcsec, filterkey='gprime'):
        ...
        pixarea = (self.plate_scale *
                   self.Camera.Detector.pixel_pitch() * 10.)**2.
    ...
    def integration_time(self, source_mag, seeing_fwhm, sky_brightness,
        ...
        Rsky = self.sky_electrons_per_pixel(
            sky_brightness, filterkey=filterkey)
...
# telescope transmission is a rough DESr-band average of the CFHT/MEGACam
# primary/corrector transmission files given in
# DESr-FullSize-woptics-watmos.par
Blanco = Telescope('-30:10:10.78', '-70:48:23.49',
                   2215, 'Blanco', 178.4 * 2., 18, 0.74)
...
LSST = Telescope('-30:14:40.7', '-70:44:57.9',
                 2662.75, 'LSST', 668, 19.64, 0.9)
